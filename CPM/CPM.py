# this class is from https://github.com/namedBen/Convolutional-Pose-Machines-Pytorch.git
...
if __name__ == "__main__":
    ...
    transform = transforms.Compose([transforms.CenterCrop(368),
                                    transforms.ToTensor(),
                                    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    img = transform(img)
    fake_center_map = img[0,:,:].unsqueeze(0).unsqueeze(0)
    img = torch.unsqueeze(img, 0)
    net = CPM(k=10)
    conv7_stage1_map, Mconv5_stage2_map, Mconv5_stage3_map,\
    Mconv5_stage4_map, Mconv5_stage5_map, Mconv5_stage6_map = net(img, fake_center_map)

    print(conv7_stage1_map.shape,
          Mconv5_stage2_map.shape, 
          Mconv5_stage3_map.shape, 
          Mconv5_stage4_map.shape, 
          Mconv5_stage5_map.shape, 
          Mconv5_stage6_map.shape)
